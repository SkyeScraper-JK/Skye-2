...
const AdminLeadsBookings: React.FC = () => {
  ...
  const handleAction = (id: string, action: string, type: 'lead' | 'booking') => {
    ...
    switch (action) {
      ...
      case 'reassign':
        if (type === 'lead') {
          navigate(`/admin/leads/${id}?action=reassign`);
        } else {
          navigate(`/admin/bookings/${id}?action=reassign`);
        }
        break;
      case 'update_status':
        if (type === 'lead') {
          navigate(`/admin/leads/${id}?action=status`);
        } else {
          navigate(`/admin/bookings/${id}?action=status`);
        }
        break;
      default:
        break;
